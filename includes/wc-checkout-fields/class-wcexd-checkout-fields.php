...
class WCEXD_Checkout_Fields {
	...
	public function add_checkout_script() {
		...
		wp_enqueue_script( 'wcexd-checkout-script', WCEXD_URI . 'includes/wc-checkout-fields/js/wcexd-checkout.js' );
		wp_localize_script(
	...
	public function set_custom_fields( $fields ) {
		...
		if ( ! empty( $this->custom_fields ) ) {
			foreach ( $this->custom_fields as $key => $value ) {
                ...
                /* Input type */
                $input_type = 'billing_wcexd_pec' === $key ? 'email' : 'text';
    ...
    public function clean_data( $document_type ) {
        ...
        if ( $document_type && 'company-invoice' !== $document_type ) {
            ...
            if ( isset( $_POST['billing_wcexd_piva'] ) ) {

                unset( $_POST['billing_wcexd_piva'] );

            }

            if ( 'private' === $document_type ) {

                if ( isset( $_POST['billing_wcexd_pec'] ) ) {

                    unset( $_POST['billing_wcexd_pec'] );

                }

                if ( isset( $_POST['billing_wcexd_pa_code'] ) ) {

                    unset( $_POST['billing_wcexd_pa_code'] );
	...
	public function checkout_fields_check() {
        ...
        $document_type = isset( $_POST['billing_wcexd_invoice_type'] ) ? sanitize_text_field( wp_unslash( $_POST['billing_wcexd_invoice_type']) ) : null;

        /* Controllo PEC */
        if ( isset( $this->custom_fields['billing_wcexd_pec'] ) ) {

            $pec = isset( $_POST['billing_wcexd_pec'] ) ? sanitize_text_field( wp_unslash( $_POST['billing_wcexd_pec'] ) ) : '';

            if ( $pec && ! is_email( $pec ) ) {

                wc_add_notice( __( 'ATTENZIONE! Sembra che la <strong>PEC</strong> inserita non sia corretta.', 'wcexd' ), 'error' );
		...
		if ( get_option( 'wcexd_fields_check' ) ) {
			...
			if ( isset( $_POST['billing_wcexd_cf'] ) && '' !== $_POST['billing_wcexd_cf'] && false === $this->fiscal_field_checker( $_POST['billing_wcexd_cf'] ) ) {
				...
				wc_add_notice( __( 'ATTENZIONE! Sembra che il <strong>Codice Fiscale</strong> inserito non sia corretto.', 'wcexd' ), 'error' );
		...
		if ( get_option( 'wcexd_vies_check' ) ) {
			...
			if ( 'company-invoice' === $document_type ) {
				...
				if ( isset( $_POST['billing_wcexd_piva'] ) && '' !== $_POST['billing_wcexd_piva'] && false === $this->vies_piva_check( $_POST['billing_wcexd_piva'], $country ) ) {
					...
					wc_add_notice( __( 'ATTENZIONE! Sembra che la <strong>Partita IVA</strong> inserita non sia corretta.', 'wcexd' ), 'error' );
	...
	public function save_fields( $order, $data ) {
		...
		if ( $this->custom_fields ) {
			foreach ( $this->custom_fields as $key => $value ) {
                ...
                if ( 'billing_wcexd_pa_code' === $key && isset( $data['billing_wcexd_pec'] ) && null ==  $data['billing_wcexd_pec']  ) {
                    if ( null == $data[ $key ] ) {
    ...
    /**
     * Recupera i custom fields effettivamente utilizzati nell'ordine
     *
     * @param int $order_id l'id dell'ordine.
     *
     * @returrn array label e valore dei custom fields
     */
    public function order_custom_fields( $order_id ) {
	...
	public function display_custom_data( $order_id ) {
        ...
        $order_custom_fields = $this->order_custom_fields( $order_id );
		...
		if ( ! empty( $order_custom_fields ) ) {
            ...
            echo '<table class="shop_table shop_table_responsive">';
                ...
                foreach ( $order_custom_fields as $key => $value ) {
                    ...
                    echo '<tr>';
                        echo '<th width="40%">' . esc_html( $key ) . ':</th>';
	...
	public function display_custom_data_in_email( $order ) {
        ...
        $order_custom_fields = $this->order_custom_fields( $order->get_id() );
		...
		if ( ! empty( $order_custom_fields ) ) {
			...
			foreach ( $order_custom_fields as $key => $value ) {
                ...
                echo '<p style="margin: 0 0 8px;">' . esc_html( $key ) . ': <span style="font-weight: normal;">' . esc_html( $value ) . '</span></p>';
